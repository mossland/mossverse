...
import { types, RenderCharacter, mapStore, gameStore } from "../../stores";
...
export const Tiles = ({ player, scope, mouse }: MapProp) => {
  ...
  const tiles = mapStore((state) => state.map?.tiles);
  const tileSize = mapStore((state) => state.map?.tileSize);
...
export const Tile = React.memo(({ offsetX, offsetY, tile, tileSize }: TileProp) => {
  ...
  const handleClick = (e: ThreeEvent<PointerEvent> | any) => {
    mapStore.setState({ pointer: e.point });
  };
  return (
    <Suspense fallback={null}>
