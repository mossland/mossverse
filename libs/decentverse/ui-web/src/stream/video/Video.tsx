...
export const Video = ({ peer, socket }: VideoProps) => {
  ...
  const mutePeer = store.callRoom.use.mutePeer();
  const unmutePeer = store.callRoom.use.unmutePeer();
  const blindPeer = store.callRoom.use.blindPeer();
  const openPeer = store.callRoom.use.openPeer();
