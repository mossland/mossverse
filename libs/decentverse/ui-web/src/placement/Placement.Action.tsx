...
interface ApproveProps {
  ...
  placement: gql.LightPlacement;
  idx?: number;
}
export const Approve = ({ slice = st.slice.placement, placement, idx }: ApproveProps) => {
  return (
    <button
      className="gap-2 btn"
      onClick={() => null}
      // onClick={() => slice.do.processPlacement(placement.id, idx)}
    >
      <NumberOutlined />
      Approve
    </button>
