...
export const MediaSettingModal = ({ isShowVideoAudioSetting }: MediaSettingModal) => {
  ...
  const localStream = store.callRoom.use.localStream();
  const audioTrack = store.callRoom.use.audioTrack();
  const videoTrack = store.callRoom.use.videoTrack();
  const videos = store.callRoom.use.videos();
  const audios = store.callRoom.use.audios();
  ...
  useEffect(() => {
    // init();
    return () => {
      console.log("unmount mediasetting modal");
      stopAudioAnalysing();
      setIsTalk(false);
    };
  }, []);
  ...
  const onClickSubmit = async () => {
    ...
    await close();
    store.callRoom.setState({ callRoom: { ...callRoom, localStream: undefined, isTalk: false } });
  };

  const close = async () => {
    store.shared.keyring.setState({ loginMethod: "none" });
    store.user.setState({ isShowVideoAudioSetting: false });
  };
        ...
        {/* <div className="select-area">
          <Field.SelectItem
            ...
            onChange={(value) => {
              ...
              store.callRoom.setState({ videoTrack: video });
            }}
          />
          <Field.SelectItem
            label="Select audio"
            ...
            onChange={(value) => {
              ...
              store.callRoom.setState({ audioTrack: audio });
            }}
          />
        </div> */}
