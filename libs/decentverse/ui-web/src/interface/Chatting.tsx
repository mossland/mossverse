...
export const Chatting = ({ socket }: ChattingProps) => {
  ...
  useEffect(() => {
    if (!self) return;
    socket.on("chat:public", (chat: gql.Chat) => {
...
export const ChatInput = ({ socket }: ChattingProps) => {
  const self = store.user.use.self();
  const chatInput = store.gossip.use.chatInput();
  const sendChat = store.gossip.use.sendChat();
  const speakChat = (chatText: string) => store.gossip.setState({ chatText });
  const onChangeChatText = (chatInput: string) => store.gossip.setState({ chatInput });
  const clearChatInput = () => store.gossip.setState({ chatInput: "" });
