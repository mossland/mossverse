...
export const WebViewModal = () => {
  ...
  const operation = store.webview.use.operation();
  const otp = store.shared.keyring.use.otp();
  const getOtp = store.shared.keyring.use.getOtp();
  ...
  return (
    <ModalWrapper>
      <ModalContainer
        ...
        closeShowModal={() => store.webview.setState({ webviewOpen: false })}
          ...
          <InnerModal>
            ...
            <TwitterWrapper>
              <TwitterTimelineEmbed
                ...
                options={{ width: "100%" }}
                onLoad={() => store.webview.setState({ operation: "idle" })}
              />
            </TwitterWrapper>
          </InnerModal>
        )}
