...
const actions = ({ set, get, pick }: SetGet<typeof state>) => ({
  ...createActions(fetch.tileGraphQL, { set, get, pick }),
  openToolBox: (position: [number, number]) => set({ tileModal: "toolBox", toolBoxPosition: position }),
  //
});

export type TileState = Get<typeof state, typeof actions>;
export type TileSlice = Slice<"tile", TileState>;
