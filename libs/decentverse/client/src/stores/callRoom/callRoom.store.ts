...
type State = Nullable<types.CallRoom> & {
  isWebviewOpen: boolean;
  audioTrack: types.Media | null;
  videoTrack: types.Media | null;
  audios: types.Media[];
  videos: types.Media[];
  CALLROOM: types.CallRoom | null;
  callRoom: types.CallBox;
...
export const useCallRoom = create<State & Action>((set, get) => ({
  ...initialState,
  forceMute: () => {
  ...
  stopAudioAnalysing: () => {
    const analyzeInterval = get().analyzeInterval;
    clearInterval(analyzeInterval);
    set({ analyzeInterval: undefined });
  },

  joinCallRoom: (roomId: string) => {
    set((state) => ({ callRoom: { ...state.callRoom, roomType: "call", roomId } }));
  },
  leaveCallRoom: () =>
...
export const callRoomStore = createSelectors(useCallRoom);
