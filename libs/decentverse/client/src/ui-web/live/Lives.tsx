...
import { liveStore, mapStore, types } from "../../stores";
...
export interface LivesProp {
  lives?: types.Live[] | null;
}
export const Lives = ({ lives = mapStore.use.lives() }: LivesProp) => {
  return (
...
export const LiveEdit = () => {
  const src = liveStore.use.src();
  return (
    <Field.Container>
      <Field.Text label="source" value={src} onChange={(src) => liveStore.setState({ src })} required />
    </Field.Container>
  );
};
...
export const LivePreview = ({ mouse }: LivePreviewProps) => {
  ...
  const lives = mapStore.use.lives();
  const update = mapStore.use.update();
  const get = liveStore.use.get();
  useEffect(() => {
  ...
  const handleClick = async (e: ThreeEvent<PointerEvent> | any) => {
    if (!point.current) point.current = [e.point.x, e.point.y];
      ...
      mapStore.setState({ lives: [...lives, { ...get(), center, wh, id: "" }] });
      plane.current?.copy(new PlaneGeometry(5, 5));
      point.current = null;
      await update();
    }
...
export const LiveList = () => {
  ...
  const handleRemove = async (live: types.Live) => {
    mapStore.setState({ lives: lives.filter((l) => l !== live) });
    await update();
  };
  return (
    <>
