...
export const SideToolBar = () => {
  const mainTool = mapStore.use.mainTool();
  const map = mapStore.use.map();
  const editMode = mapStore.use.editMode();
      ...
      {map && (
        <>
          <Menu
            ...
            onSelect={({ key }) => mapStore.setState({ editMode: key as types.EditMode, edit: null })}
          >
          ...
          {editMode === "add" && (
            <>
              <Menu
                ...
                onSelect={({ key }) => mapStore.setState({ mainTool: key as types.MainTool })}
              >
