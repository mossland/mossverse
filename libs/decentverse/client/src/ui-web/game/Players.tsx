...
import { types, worldStore, gameStore, emojiStore, mapStore } from "../../stores";
...
export const Players = ({ playerId }: PlayersProp) => {
  const otherPlayers = worldStore.use.otherPlayers();
  const otherPlayerIds = worldStore.use.otherPlayerIds();
  return (
    <>
...
export const OtherPlayer = React.memo(({ player, texture }: OtherPlayerProp) => {
  ...
  const animation = useRef<types.SpriteDef>(player.character.right.idle);
    ...
    const subscription = (id: string, data: types.RenderOtherPlayer) => {
      ...
      if (!movement.current) {
        ...
        if (
          ...
          data.effect !== movement.current.prev.effect
        )
  ...
  return (
    <sprite ref={sprite} position={sprite.current?.position}>
      <mesh
        ...
        onClick={(event) => {
          worldStore.setState({ otherPlayer: player });
          gameStore.setState({ pointer: [event.nativeEvent.clientX, event.nativeEvent.clientY] });
          lockKey(true);
        }}
      >
...
export const PlayerEventRender = ({ id, playerSize }: { id: string; playerSize: [number, number] }) => {
  ...
  useEffect(() => {
    const subscription = (
      _: string,
      state: { chatText: string; isTalk: boolean; emoji: string; effect: types.EffectType }
    ) => {
      setChatState(state);
    };
  ...
  return (
    <>
      ...
      <Effect effectType={chatState.effect} />
    </>
  );
};
