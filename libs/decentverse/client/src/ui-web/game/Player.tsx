...
export interface PlayerProp {
  ...
  animation: MutableRefObject<types.SpriteDef>;
  keyboard: MutableRefObject<types.Keyboard>;
...
export const Player = React.memo(({ sprite, animation, keyboard, player, engine, zoom }: PlayerProp) => {
  ...
  const saveUser = userStore.use.saveUser();
...
const MyEmoji = () => {
  const character = worldStore((state) => state.me.character);
  const keyboard = gameStore.use.keyboard();
  // const checkEmoji = emojiStore.use.checkEmoji();
  const url = emojiStore((state) => state.emoji?.file.url);
  const emoji = emojiStore.use.emoji();
