...
const state = ({ set, get, pick }: SetGet<slice.CallRoomSliceState>) => ({
  ...slice.makeCallRoomSlice({ set, get, pick }),
...
export type CallRoomState = State<typeof state, typeof actions>;
// * 3. ChildSlice를 추가하세요. Suffix 규칙은 일반적으로 "InModel" as const 로 작성합니다.
export const addCallRoomToStore = ({ set, get, pick }: SetGet<CallRoomState>) => ({
  ...state({ set, get, pick }),
