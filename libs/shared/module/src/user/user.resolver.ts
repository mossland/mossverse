...
export class UserResolver extends BaseResolver(gql.User, gql.UserInput, Allow.Public, Allow.Public, Allow.Owner) {
  ...
  @UseGuards(Allow.User)
  @Mutation(() => gql.User)
  async removeUser(@Args({ name: `userId`, type: () => ID }) userId: string, @Auth() account: Account | null) {
    if (!account || !account._id.equals(userId)) throw new Error("Not authorized");
    return await this.userService.remove(new Id(userId));
  }
