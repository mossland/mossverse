...
describe("Contract Service", () => {
  ...
  beforeAll(async () => {
    ...
    etherService = app.get<EtherService>(EtherService);
    ...
    etherWallets = system.env.network.klaytn.testWallets.map(
      (w) => new ethers.Wallet(w.privateKey, etherService.provider)
    );
  }, 10000);
  ...
  it("Create ERC-20 Contract", async () => {
    input = sample.contractInput(network._id, system.env.network.klaytn.erc20);
  ...
  it("Snapshot ERC20 Contract", async () => {
    ...
    await erc20contract.merge({ snapshot: null }).save();
  ...
  it("Detect Transfer of ERC20", async () => {
    ...
    const num = testWallets[0].items.find((item) => item.contract.equals(erc20contract._id)).num;
    await instance.contract.transfer(testWallets[0].address, 10);
    await Utils.sleep(8000);
    await testWallets[0].refresh();
    expect(testWallets[0].items.find((item) => item.contract.equals(erc20contract._id)).num).toEqual(num + 10);
  ...
  it("Create ERC-721 Contract", async () => {
    input = sample.contractInput(network._id, system.env.network.klaytn.erc721);
  ...
  it("Snapshot ERC721 Contract", async () => {
    await wallet.merge({ items: [] }).save();
    await erc721contract.merge({ snapshot: null }).save();
  ...
  it("Detect Transfer of ERC721", async () => {
    ...
    await instance.contract.transferFrom(wallet.address, testWallets[0].address, token.tokenId, { gasLimit: 300000 });
    await Utils.sleep(8000);
    await testWallets[0].refresh();
    ...
    await instance.contract
      ...
      .transferFrom(testWallets[0].address, wallet.address, token.tokenId, { gasLimit: 300000 });
  }, 20000);
  ...
  it("Create ERC-1155 Contract", async () => {
    input = sample.contractInput(network._id, system.env.network.klaytn.erc1155);
  ...
  it("Snapshot ERC1155 Contract", async () => {
    ...
    await Utils.sleep(8000);
    await wallet.merge({ items: [] }).save();
    await erc1155contract.merge({ snapshot: null }).save();
  ...
  it("Detect Transfer of ERC1155", async () => {
    ...
    await Utils.sleep(2000);
    await testWallets[0].refresh();
    const num = testWallets[0].items.find((item) => item.token.equals(token._id)).num;
    await instance.contract.safeTransferFrom(wallet.address, testWallets[0].address, token.tokenId, 10, "0x00");
    await Utils.sleep(8000);
    await testWallets[0].refresh();
    expect(testWallets[0].items.find((item) => item.token.equals(token._id)).num).toEqual(num + 10);
    await instance.contract
      .connect(etherWallets[0])
      .safeTransferFrom(testWallets[0].address, wallet.address, token.tokenId, 11, "0x00");
  }, 20000);
  ...
  it("Update Inventory of Wallet", async () => {
    erc20contract = await contractService.generateContract(
      sample.contractInput(network._id, system.env.network.klaytn.erc20)
    );
    erc721contract = await contractService.generateContract(
      sample.contractInput(network._id, system.env.network.klaytn.erc721)
    );
    erc1155contract = await contractService.generateContract(
      sample.contractInput(network._id, system.env.network.klaytn.erc1155)
    );
    // const instance = (await networkService.loadContract(erc1155contract)) as Erc1155;
    // await instance.contract.safeTransferFrom(wallet.address, testWallets[0].address, 0, 1, "0x00");
