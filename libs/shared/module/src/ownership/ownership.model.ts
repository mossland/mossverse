...
import { OwnershipSchema, OwnershipInput } from "./ownership.gql";
...
export type ThingInput = { user: Id; thing: db.Thing.Doc; value: number };
export type TokenInput = { user?: Id; wallet?: Id; token: db.Token.Doc; value: number };
...
interface MdlStats extends dbConfig.DefaultMdlStats<Doc, Raw> {
  ...
  transferThing: (thing: db.Thing.Doc, from: Id, to: Id, value: number) => Promise<Doc[]>;
  transferToken: (token: db.Token.Doc, from: Id, to: Id, value: number, bn?: number) => Promise<Doc[]>;
}
...
schema.statics.browseToken = async function (this: Mdl, data: TokenInput) {
  const query = { wallet: data.wallet, type: "token", token: data.token._id, status: "active" };
...
schema.statics.transferThing = async function (this: Mdl, thing: db.Thing.Doc, from: Id, to: Id, value: number) {
