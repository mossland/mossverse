...
import { File, fileFragment } from "../file/file.fetch";
import { OpenSeaMeta } from "../_shared/shared.fetch";
...
export class Token extends BaseGql(TokenInput) {
  ...
  static addOrRemoveAttributeFilterQuery(queryOfToken: any, traitType: string, value: string) {
    ...
    if (query?.["meta.attributes.value"].$in.includes(value))
        ...
        q === query
          ? {
              ...
              "meta.attributes.value": {
                $in: [...query["meta.attributes.value"].$in, value],
              },
            }
          : q
      );
      ...
      queries.push({
        "meta.attributes.trait_type": traitType,
        "meta.attributes.value": { $in: [value] },
      });
    const andQuery = [...queries.filter((query) => query["meta.attributes.value"].$in.length > 0)];
    return {
      ...queryOfToken,
      tokenId: undefined,
      $and: andQuery.length ? andQuery : undefined,
    };
  }
...
export const tokenQueryMap = {
  totalToken: { status: { $ne: "inactive" } },
};
...
export const addTokenFiles = async (files: FileList, tokenId?: string) =>
  (
    await mutate<AddTokenFilesMutation>(addTokenFilesMutation, {
      files,
      tokenId,
    })
  ).addTokenFiles;
...
export const generateToken = async (contractId: string, tokenId: number) =>
  (
    await mutate<GenerateTokenMutation>(generateTokenMutation, {
      contractId,
      tokenId,
    })
  ).generateToken;
