...
const state = (setget) => ({
  ...createState(fetch.networkGraphQL),
});

// * 2. Action을 내용을 정의하세요. Action은 모두 void 함수여야 합니다.
// * 다른 action을 참조 시 get() as <Model>State 또는 RootState 를 사용하세요.
const actions = ({ set, get, pick }: SetGet<typeof state>) => ({
  ...createActions(fetch.networkGraphQL, { set, get, pick }),
  //
  //
});

export type NetworkState = Get<typeof state, typeof actions>;
export type NetworkSlice = Slice<"network", NetworkState>;
