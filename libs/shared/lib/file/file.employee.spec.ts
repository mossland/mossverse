...
describe("File Service", () => {
  ...
  let fileEmployee: FileEmployee;
  let s3Service: emp.external.S3Service;
  beforeAll(async () => {
    const app = await system.init(registerModules(environment));
    fileEmployee = app.get<FileEmployee>(FileEmployee);
    s3Service = app.get<emp.external.S3Service>(emp.external.S3Service);
  });
  afterAll(async () => await system.terminate());
  let file: db.File.Doc;
  let input: cnst.FileStream;
  it("Create File", async () => {
    input = sample.fileStream();
    [file] = await fileEmployee.addFiles([input], "test", "test");
  ...
  it("Remove File", async () => {
    file = await fileEmployee.remove(file._id);
  ...
  it("Get Json from URI", async () => {
    ...
    const data = await fileEmployee.getJsonFromUri(uri);
  ...
  it("Create File from URI", async () => {
    ...
    const file = await fileEmployee.addFileFromUri(uri, "test", "test");
