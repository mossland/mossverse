...
const state = ({ set, get, pick }: SetGet<slice.TokenSliceState>) => ({
  ...slice.makeTokenSlice({ set, get, pick }),
  tokenSwipeIndex: 0,
});
...
const actions = ({ set, get, pick }: SetGet<typeof state>) => ({
  selectAttributeFilter: async (traitType: string, value: string) => {
    ...
    await setQueryOfToken(gql.Token.addOrRemoveAttributeFilterQuery(queryOfToken, traitType, value));
    set({ tokenSwipeIndex: 0 });
  },
  resetAttributeFilterByTraitType: async (traitType: string) => {
    const { queryOfToken, setQueryOfToken } = get() as Get<typeof state, typeof actions>;
    await setQueryOfToken(gql.Token.removeAttributeFilterQueryByTraitType(queryOfToken, traitType));
    set({ tokenSwipeIndex: 0 });
  },
});

export type TokenState = State<typeof state, typeof actions>;
// * 3. ChildSlice를 추가하세요. Suffix 규칙은 일반적으로 "InModel" as const 로 작성합니다.
export const addTokenToStore = ({ set, get, pick }: SetGet<TokenState>) => ({
  ...state({ set, get, pick }),
