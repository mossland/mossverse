...
import { createGraphQL, Field, InputType, ObjectType, BaseGql, Int, ID, Float, PickType } from "@shared/util-client";
...
export class Ownership extends BaseGql(OwnershipInput) {
  ...
  static get(ownershipList: LightOwnership[], name: string) {
  ...
  static getValue(ownershipList: LightOwnership[], name: string) {
    return this.get(ownershipList, name)?.value || 0;
  }
  static getCredit(ownershipList: LightOwnership[], name: string) {
    return this.get(ownershipList, name)?.credit || 0;
  }
