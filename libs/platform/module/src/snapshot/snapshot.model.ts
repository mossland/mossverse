...
import { SnapshotSchema, SnapshotInput } from "./snapshot.gql";
...
schema.statics.archive = async function (archiveType: "periodic" | "non-periodic", data: Input) {
  ...
  await this.updateOne(
    { status: "active", type: "active", target: data.target, targetType: data.targetType },
    { ...data, type: "active", at, status: "active" },
    { upsert: true }
  );
  await this.updateOne(
    { status: "archived", target: data.target, targetType: data.targetType, type, at },
    { ...data, type, at, status: "archived" },
    { upsert: true }
  );
  return await this.pickOne({ status: "archived", type, at, target: data.target, targetType: data.targetType });
};
