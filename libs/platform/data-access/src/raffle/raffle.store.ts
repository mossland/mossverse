...
const state = ({ set, get, pick }: SetGet<slice.RaffleSliceState>) => ({
  ...slice.makeRaffleSlice({ set, get, pick }),
});
...
const actions = ({ set, get, pick }: SetGet<typeof state>) => ({
  requestRaffle: async () => {
    const {
      ...
      refreshRaffle,
      queryOfRaffle,
      refreshOwnershipInItem,
      refreshOwnershipInMoney,
    } = get() as RootState;
    if (raffle === "loading") return;
    const priceTag = await gql.purifyPriceTag(raffle.priceTags[0]);
    if (!priceTag) throw new Error("priceTag is not found");
    await gql.raffle(raffle.id, priceTag);
...
export type RaffleState = State<typeof state, typeof actions>;
// * 3. ChildSlice를 추가하세요. Suffix 규칙은 일반적으로 "InModel" as const 로 작성합니다.
export const addRaffleToStore = ({ set, get, pick }: SetGet<RaffleState>) => ({
  ...state({ set, get, pick }),
