...
import { Entry, PriceTag } from "./../_scalar";
import { gql as shared } from "@shared/data-access";
import { gql as platform } from "@platform/data-access";
import dayjs, { Dayjs } from "dayjs";
// import { User } from "@shared/module/db";
...
export class Raffle extends BaseGql(RaffleInput) {
  ...
  entryNum(self: shared.User) {
    return this.entries.find((e) => e.user.id === self.id)?.value;
  }

  isPicked(self: shared.User) {
    return this.winners.every((e) => e.id === self.id);
  }
}
