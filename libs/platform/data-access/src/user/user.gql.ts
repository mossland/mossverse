...
export class User extends UserInput {
  ...
  // getThings(names: string[]): shared.ThingItem[] {
  //   // filter names from user.items
  //   return this.items.filter((item) => names.includes(item.thing.name));
  // }
  // static getThings(user: User, names: string[]): shared.ThingItem[] {
  //   // filter names from user.items
  //   return user.items.filter((item) => names.includes(item.thing.name));
  // }

  filterMyItems(self: User, listings: platform.Listing[]) {
