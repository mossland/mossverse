...
import { TradeInput, TradeSchema } from "./trade.constant";
...
interface DocMtds extends dbConfig.DefaultDocMtds<Doc> {
  makeExchange: (
    ...
    reverse: boolean
  ) => [cnst.ExchangeInput[], cnst.ExchangeInput[]];
}
schema.methods.makeExchange = function (
  ...
  const inputs: cnst.ExchangeInput[] = is.map((input) => {
  ...
  const outputs: cnst.ExchangeInput[] = os.map((output) => {
    ...
    return {
      ...desired,
      ...output,
      value: output.value * multiple,
      wallet: desired.wallet,
    };
  });
