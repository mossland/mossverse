...
export const SellBox = () => {
  ...
  const listingForm = store.platform.listing.use.listingForm();
    ...
    <SellBoxContainer>
              ...
              .map((item, index) => (
                <div key={item.thing.id}>
                  ...
                  <input
                    type="text"
                    id="price"
                    value={store.platform.listing.use.listingForm.priceTags[0]?.price ?? ""}
                    min={1}
                    max={myItem ? myItem.num : 1}
                    onChange={(e) => {
                      store.platform.listing.set({
                        priceTag: {
                          // id: "",
                          token: null,
                          thing: item.thing,
      ...
      <div className="form-box">
        ...
        <DatePicker
          ...
          value={.closeAt.getTime() > 0 ? moment(listingForm.closeAt) : moment(maxDate)}
          onChange={(e) => store.platform.listing.do.setListingForm({ closeAt: e ? e.toDate() : new Date(-1) } )}
        />
      </div>
      {listingForm ? (
        ...
        <div
          className={`button button--sell ${(!listingForm.priceTags[0]?.price || !listingForm.closeAt) && "disabled"}`}
