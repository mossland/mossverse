...
import { MocSurveyInput, MocSurveySchema } from "./mocSurvey.constant";
...
interface DocMtds extends dbConfig.DefaultDocMtds<Doc> {
  getResponseUsers: () => Id[];
  addResponse: (response: cnst.platform.UserSurveyResponse) => Doc;
  removeResponse: (response: cnst.platform.UserSurveyResponse) => Doc;
  open: () => Doc;
  close: (snapshots: { user: Id; num: number }[]) => Doc;
}
schema.methods.addResponse = function (this: Doc, response: cnst.platform.UserSurveyResponse) {
...
schema.methods.removeResponse = function (this: Doc, response: cnst.platform.UserSurveyResponse) {
...
schema.methods.open = function (this: Doc) {
  if (this.status !== "applied") throw new Error("Survey is not applied");
  (this.constructor as Mdl).moveSummary(this.status, "surveying");
  this.status = "surveying";
  return this;
};
schema.methods.close = function (this: Doc, ownerships: cnst.platform.MocOwnership[]) {
  const selectNum = new Array(this.selections.length).fill(0);
  this.merge({
    userNum: 0,
    mocNum: 0,
    selectMocNum: selectNum,
    selectUserNum: selectNum,
    responses: [],
  });
  const ownerMap = new Map<string, cnst.platform.MocOwnership>();
  ...
  this.responses = this.responses.map((response) => {
    const snapshot = ownerMap.get(response.user.toString()) ?? {
      user: response.user,
      num: 0,
    };
    const res: cnst.platform.UserSurveyResponse = {
      ...response,
      user: snapshot.user,
      num: snapshot.num,
    };
    ...
    this.mocNum += res.num;
    this.userNum += 1;
    (this.constructor as Mdl).moveSummary(this.status, "closed");
    return res;
  });
  Object.assign(this, {
    snapshot: ownerships,
